...
class BattleRepositoryImpl(
    ...
    private fun BattleRemoteSource.throwIfExistBattle(t: Transaction) {
        ...
        val battle = getBattle(t, participantBattleId)?.toDomain2() ?: return
        if (battle !is BattleEntity.Closed)
            throw Exception("이미 생성된 게임(${battle.id})이 있습니다. 생성된 게임을 종료 후 다시 시도 해 주세요.")

//        val participantEntity = participant.toDomain2(battle)


//            ?.battleId
//            ?.also {
//                getBattle(t, it)
//                    ?.run { throw Exception("이미 생성된 게임($it)이 있습니다. 생성된 게임을 종료 후 다시 시도 해 주세요.") }
//            }
    }
    ...
    override suspend fun list() = battleRemoteSource
        .getBattleList(10, -1)
        .mapNotNull { it.runCatching { toDomain2() }.getOrNull() }
    ...
    private suspend fun doJoinBattle(battleId: String) {
        FirebaseFirestore.getInstance().runTransaction { t ->
            with(battleRemoteSource) {
                ...
                val battleEntity = when (val entity = battle.toDomain2()) {
...
//                        return@forEach
//
//                    val task = FirebaseFirestore.getInstance().runTransaction { t ->
//                        // 참여자가 존재하는 경우 아무 참여자나 1명을 host 로 변경하여 준다.
//                        val modifyBattle = mapOf(
//                            "hostUid" to first().uid,
//                            "participantSize" to size
//                        )
//                        with(battleRemoteSource) {
//                            updateBattle(t, battle.id, modifyBattle)
//                            setParticipant(t, first().uid, mapOf("isReady" to true))
//                        }
//                    }
//
//                    isSuccess = task
...
//                val participantModel = getParticipant(t, uid) ?: return@runTransaction
        ...
        val participantList = battleRemoteSource
            .getParticipantList(battleId)
            .map { it.toDomain() }
//        val participantList = getParticipantList(battleId)
        FirebaseFirestore.getInstance().runTransaction { t ->
    ...
    override suspend fun getStatistics() =
        battleRemoteSource.getStatistics(currentUserUid).toDomain()
}